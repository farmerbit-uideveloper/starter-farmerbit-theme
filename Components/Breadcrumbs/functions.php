        ...
        if ( is_home() || is_front_page() ) {
            ...
            if ( is_category() ) {
                ...
                if ( get_post_type() == 'post' ) {
                    ...
                    $page_blog = Options::getTranslatable('ImpostazioniSito', 'pageBlog');
                    $breadcrumbs_echo .= sprintf( $link, get_permalink($page_blog->ID), get_the_title($page_blog->ID), $position );
                ...
                } elseif ( get_post_type() == 'ospedale' ) {
                    $position += 1;
                    $breadcrumbs_echo .= $sep;
                    $page_blog = Options::getTranslatable('ImpostazioniSito', 'pageOspedale');
                    $breadcrumbs_echo .= sprintf( $link, get_permalink($page_blog->ID), get_the_title($page_blog->ID), $position );
                    if ( get_query_var( 'cpage' ) ) {
                        $position += 1;
                        $breadcrumbs_echo .= $sep . sprintf( $link, get_permalink(), get_the_title(), $position );
                ...
                } elseif ( get_post_type() == 'casa-di-riposo' ) {
                    $position += 1;
                    $breadcrumbs_echo .= $sep;
                    $page_blog = Options::getTranslatable('ImpostazioniSito', 'pageCasadiRiposo');
                    $breadcrumbs_echo .= sprintf( $link, get_permalink($page_blog->ID), get_the_title($page_blog->ID), $position );
                    if ( get_query_var( 'cpage' ) ) {
                        $position += 1;
                        $breadcrumbs_echo .= $sep . sprintf( $link, get_permalink(), get_the_title(), $position );
                ...
                if ( get_query_var( 'paged' ) ) {
                    $position += 1;
                    if ( $position > 1 ) $breadcrumbs_echo .= $sep;
                    $breadcrumbs_echo .= sprintf( $link, get_post_type_archive_link( $post_type->name ), $post_type->label, $position );
                    $breadcrumbs_echo .= $sep . $before . sprintf( $text['page'], get_query_var( 'paged' ) ) . $after;
                } else {
                    if ( $show_home_link && $show_current ) $breadcrumbs_echo .= $sep;
                    if ( $show_current ) $breadcrumbs_echo .= $before . $post_type->label . $after;
                    elseif ( $show_home_link && $show_last_sep ) $breadcrumbs_echo .= $sep;
                }
            ...
            return $breadcrumbs_echo;
        }
    }

    $data['breadcrumbs'] = dimox_breadcrumbs();
